...
public class VerifySelectPickupScreenByGpsDisabled 
{
	private ExtLogger log = new ExtLogger(VerifySelectPickupScreenByGpsDisabled.class.toString());
	...
	String moduleName = "Verify Search Locationsby GPS Disabled";
	...
	@Test
	public void SearchLocationbyGPSDisable()
	{
		try {
            ...
            try {
				...
				reports.writeIntoFile(driver, testCaseName,  "Verify pickup title", "Verify pickup title","title is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify pickup title", "Verify pickup title", "title is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
