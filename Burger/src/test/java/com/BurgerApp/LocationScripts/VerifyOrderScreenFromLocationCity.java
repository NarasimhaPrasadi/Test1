...
public class VerifyOrderScreenFromLocationCity 
{
	private ExtLogger log = new ExtLogger(VerifyOrderScreenFromLocationCity.class.toString());
	...
	String moduleName = "Verify Oder Screen From Location City";
	...
	@Test
	public void VerifyOrderScreenFromLocationByCity()
	{
		try {
           ...
           try {
        	    Assert.assertTrue(locations.isPickUpMethodPopupDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Tap on Search key", "Verify Pick Up Method","Pick Up Method Links Verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Tap on Search key", "Verify Pick Up Method", "Pick Up Method Links Verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Shared Links" +e.getMessage());
				Assert.assertTrue(false);
			}
           locations.DineIn();
