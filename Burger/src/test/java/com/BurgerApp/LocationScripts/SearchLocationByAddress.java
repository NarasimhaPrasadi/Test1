...
public class SearchLocationByAddress 
{
	private ExtLogger log = new ExtLogger(SearchLocationByAddress.class.toString());
	...
	String moduleName = "Verify Location by Address";
	...
	public void startUp() throws Exception {
		...
		if(datamanager.executionController(testCaseName).equals("Yes")) {
			...
			Runtime.getRuntime().exec("adb shell settings put secure location_providers_allowed -gps");
			Runtime.getRuntime().exec("adb shell settings put secure location_providers_allowed -network");
	...
	@Test
	public void VerifyLocationbyAddress()
	{
		try {
            ...
            locations.LocationSearchByAddress();
            comlib.tapSearchKey(driver);
            
            try {
            	Assert.assertTrue(locations.isLocatedBurgerStoreAddress());
            ...
            locations.LocatedBurgerStoreAddress();
            locations.addressLayoutTap();  
            
            try {
            	Assert.assertTrue(locations.isBullardAddressDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify Address", "Verify Address","Address is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify Address", "Verify Address", "Address is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Address" +e.getMessage());
				Assert.assertTrue(false);
			}
            
            try {
            	Assert.assertTrue(locations.isBullardTXDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify BullardTX", "Verify BullardTX","BullardTX is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify BullardTX", "Verify BullardTX", "BullardTX is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in BullardTX" +e.getMessage());
				Assert.assertTrue(false);
			}
            
            try {
            	Assert.assertTrue(locations.isMobileNoDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileNo", "Verify MobileNo","MobileNo is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileNo", "Verify MobileNo", "MobileNo is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in MobileNo" +e.getMessage());
				Assert.assertTrue(false);
			}
				...
				log.stepLog("Exception in AddressLocation" +e.getMessage());
				Assert.assertTrue(false);
		}
	 }
