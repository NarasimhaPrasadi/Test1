...
public class VerifyFBWithoutCredentialSettingsScreen 
{
	private ExtLogger log = new ExtLogger(VerifyFBWithoutCredentialSettingsScreen.class.toString());
	...
	String moduleName = "Verify FBLoginSettingsScreen";
	...
	@Test
	public void VerifySettingsFacebook()
	{
		try {
            ...
            try {
                Assert.assertTrue(home.isfacebookButtonDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Tap on Settings Icon", "Tap on Settings Icon","Settings screen is verified with facebook Icon", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Tap on Settings Icon", "Tap on Settings Icon", "Settings screen is verified with facebook Icon", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
            
            home.facebookTap();
            home.FbSettingsPopUp();
