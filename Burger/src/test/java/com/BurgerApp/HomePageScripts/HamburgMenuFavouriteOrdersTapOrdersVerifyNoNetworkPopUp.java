...
public class HamburgMenuFavouriteOrdersTapOrdersVerifyNoNetworkPopUp 
{
	private ExtLogger log = new ExtLogger(HamburgMenuFavouriteOrdersTapOrdersVerifyNoNetworkPopUp.class.toString());
	...
	@Test
	public void HamburgMenuFavouriteOrdersTapOrdersVerifyNoNetworkPopUpScript()
	{
		try {
			...
			home.SignInScreenLoginWithCredential("sudhir01pandit@gmail.com", "burger01");
			...
			try {
        	    Assert.assertTrue(home.isMenuListDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify MenuListDisplayed", "Verify MenuListDisplayed", "MenuListDisplayed is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MenuListDisplayed", "Verify MenuListDisplayed", "MenuListDisplayed is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}     
			
			comlib.swipeuntilTextMatched(driver, "Edit Favorite Orders");
