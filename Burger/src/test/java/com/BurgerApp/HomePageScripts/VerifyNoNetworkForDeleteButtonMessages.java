...
public class VerifyNoNetworkForDeleteButtonMessages 
{
	private ExtLogger log = new ExtLogger(VerifyNoNetworkForDeleteButtonMessages.class.toString());
	...
	@Test
	public void VerifyNoNetworkForDeleteButtonMessagesScript()
	{
		try {
			...
			try {
				Assert.assertTrue(home.isDeleteButtonDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify DeleteButton", "Tap on DeleteButton", "DeleteButton is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify DeleteButton", "Tap on DeleteButton", "DeleteButton is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
			    
			home.DeleteButtonTapMessage();
