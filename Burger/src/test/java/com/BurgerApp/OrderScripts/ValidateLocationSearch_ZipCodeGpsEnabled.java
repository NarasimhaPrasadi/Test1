...
public class ValidateLocationSearch_ZipCodeGpsEnabled 
{
	private ExtLogger log = new ExtLogger(ValidateLocationSearch_ZipCodeGpsEnabled.class.toString());
	...
	String moduleName = "Verify Location by Zip";
	...
	@Test
	public void ValidateLocationSearch_ZipCodeGpsEnabledScript()
	{
		try {
			...
			home.Fbskip();
            home.Orders();
            home.GpsEnabledAlertPopup();
            
            try {
            	Assert.assertTrue(locations.isSelectLocationPickupTitleDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify Select LocationPickup Screen", "Verify Select LocationPickup SCreen","SelectLocationPickupTitle is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify Select LocationPickup Screen", "Verify SelectLocationPickup Screen", "SelectLocationPickupTitle is verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}      
            ...
            try {
				...
				reports.writeIntoFile(driver, testCaseName,  "Tap on Search key", "Verify Located store","Location is clickable", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Tap on Search key", "Verify Located store", "Location is clickable", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}      
