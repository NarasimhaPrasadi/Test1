...
public class ValidateLocationPickUpScreenUI 
{
	private ExtLogger log = new ExtLogger(ValidateLocationPickUpScreenUI.class.toString());
	...
	String moduleName = "Verify Driver thru";
	...
	@Test
	public void ValidateLocationPickUpScreenUIScript()
	{
		...
		try {
            ...
            try {
                 ...
                 Assert.assertTrue(locations.isNearByTextDisplayed());
                 Assert.assertTrue(locations.isFavouriteTextDisplayed());
                 Assert.assertTrue(locations.isselectPickupMethodTitleDisplayed());
                 Assert.assertTrue(locations.isrefreshButtonLocated());
				reports.writeIntoFile(driver, testCaseName,  "Verify UI", "Verify UI", "UI is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify UI", "Verify UI", "UI is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
