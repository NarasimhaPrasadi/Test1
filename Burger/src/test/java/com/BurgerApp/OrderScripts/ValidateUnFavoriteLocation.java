...
public class ValidateUnFavoriteLocation 
{
	private ExtLogger log = new ExtLogger(ValidateUnFavoriteLocation.class.toString());
	...
	@Test
	public void ValidateUnFavoriteLocationScript()
	{
		...
		try {
            ...
            try {
            	Assert.assertTrue(locations.isNoFavoritelocationsDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify NoFavoritelocations", "Verify NoFavoritelocations", "NoFavoritelocations is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify NoFavoritelocations", "Verify NoFavoritelocations", "NoFavoritelocations is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}           
