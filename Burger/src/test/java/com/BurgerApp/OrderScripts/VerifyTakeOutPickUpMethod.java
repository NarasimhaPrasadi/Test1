...
public class VerifyTakeOutPickUpMethod 
{
	private ExtLogger log = new ExtLogger(VerifyTakeOutPickUpMethod.class.toString());
	...
	String moduleName = "Verify TakeOut";
	...
	public void VerifyTakeOut()
	{
		...
		try {
           ...
           try {
            ...
            locations.OrderMenutap();
            locations.CustomizeScreenDisplayed();
            comlib.swipeuntilText(driver, "Continue"); 
            locations.Continue();
            comlib.swipeuntil(driver, "Add to My Order");
            locations.globalNegativeButton();
            locations.TakeOut();
            locations.EditButton();
            
            try {
                Assert.assertTrue(locations.isTakeOutDisplayed());
 				reports.writeIntoFile(driver, testCaseName,  "Verify Take Out", "Verify Take Out","Take Out Screen Verified", reports.pass, "", comlib.getCurrentTime());
 			}
 			catch(Exception e)
 			{
 				reports.writeIntoFile(driver, testCaseName,  "Verify Take Out", "Verify Take Out", "Take Out Screen Verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
 				log.stepLog("Exception in My Order Screen" +e.getMessage());
 				Assert.assertTrue(false);
 			}
