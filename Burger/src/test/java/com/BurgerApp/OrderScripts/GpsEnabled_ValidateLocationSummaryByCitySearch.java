...
public class GpsEnabled_ValidateLocationSummaryByCitySearch 
{
	private ExtLogger log = new ExtLogger(GpsEnabled_ValidateLocationSummaryByCitySearch.class.toString());
	...
	@Test
	public void GpsEnabled_ValidateLocationSummaryByCitySearchScript()
	{
		try {
            ...
            try {
                Assert.assertTrue(locations.isMilesLayoutDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify MilesLayout", "Verify MilesLayout","MilesLayout is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MilesLayout", "Verify MilesLayout", "MilesLayout not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
            locations.MobileOrderingPopUp();
