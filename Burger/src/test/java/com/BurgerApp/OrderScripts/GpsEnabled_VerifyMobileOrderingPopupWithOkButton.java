...
public class GpsEnabled_VerifyMobileOrderingPopupWithOkButton 
{
	private ExtLogger log = new ExtLogger(GpsEnabled_VerifyMobileOrderingPopupWithOkButton.class.toString());
	...
	@Test
	public void GpsEnabled_VerifyMobileOrderingPopup()
	{
		try {
            ...
            try {
				...
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileOrder popup", "Verify MobileOrder popup","MobileOrder popup is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileOrder popup", "Verify MobileOrder popup", "MobileOrder not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
