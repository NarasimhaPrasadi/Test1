...
public class GpsDisabled_WifiOff_RefreshTapVerifyNoNetworkConnection 
{
	private ExtLogger log = new ExtLogger(GpsDisabled_WifiOff_RefreshTapVerifyNoNetworkConnection.class.toString());
	...
	String moduleName = "Verify GpsEnabled_Wifi_Off_NetworkPopup";
	...
	@Test
	public void GpsDisabled_WifiOff_RefreshTapVerifyNoNetworkConnectionScript()
	{
		try {
	        ...
	        home.OkGotItTap();
	        locations.RefreshButtonTap();        
	        
	        try {
        	    Assert.assertTrue(locations.isGlobalTitleDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify GlobalTitle", "Verify GlobalTitle", "Popup is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify GlobalTitle", "Verify GlobalTitle", "Popup is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
