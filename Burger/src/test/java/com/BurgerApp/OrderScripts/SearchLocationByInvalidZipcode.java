...
public class SearchLocationByInvalidZipcode 
{
	private ExtLogger log = new ExtLogger(SearchLocationByInvalidZipcode.class.toString());
	...
	@Test
	public void SearchLocationByInvalidZipcode_Script()
	{
		...
		try {
            ...
            locations.LocationSearchByInValidZipCode();
            comlib.tapSearchKey(driver);
            
            try {
				Assert.assertTrue(locations.isNoLocationsPopupDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify No location popup", "Tap on Ok button", "PopUp is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify No location popup", "Tap on Ok button", "PopUp is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
