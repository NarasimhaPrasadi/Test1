...
public class VerifyDineInPickUpMethod 
{
	private ExtLogger log = new ExtLogger(VerifyDineInPickUpMethod.class.toString());
	...
	String moduleName = "Verify DineIn";
           ...
           try {
            ...
            try {
                Assert.assertTrue(locations.isDineInDisplayed());
 				reports.writeIntoFile(driver, testCaseName,  "Verify DineIn", "Verify DineIn","DineIn Screen Verified", reports.pass, "", comlib.getCurrentTime());
 			}
 			catch(Exception e)
 			{
 				reports.writeIntoFile(driver, testCaseName,  "Verify DineIn", "Verify DineIn", "DineIn Screen Verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
