...
public class ValidateDistanceFromPin_Miles 
{
	private ExtLogger log = new ExtLogger(ValidateDistanceFromPin_Miles.class.toString());
	...
	@Test
	public void ValidateDistanceFromPin_MilesScript()
	{
		...
		try {
            ...
            home.Orders();
            home.startNewOrder();
            locations.AustinStoreByFakeGps(); 
			...
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MilesLayout", "Verify MilesLayout", "MilesLayout is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
