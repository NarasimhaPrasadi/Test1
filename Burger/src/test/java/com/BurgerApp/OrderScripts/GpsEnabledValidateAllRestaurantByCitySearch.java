...
public class GpsEnabledValidateAllRestaurantByCitySearch 
{
	private ExtLogger log = new ExtLogger(GpsEnabledValidateAllRestaurantByCitySearch.class.toString());
	...
	String moduleName = "VerifyMobileOrderingPopupWithOkButton";
	...
	@Test
	public void GpsEnabledValidateAllRestaurantByCitySearchScript()
	{
		try {
            ...
            try {
            	Assert.assertTrue(locations.isLocationStoresDisplayedForCityAustin());
				reports.writeIntoFile(driver, testCaseName,  "Verify All restaurant", "Verify All restaurant","All restaurant is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify All restaurant", "Verify All restaurant", "All restaurant is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
            
            locations.MobileOrderingPopUp();
            
            try {
            	Assert.assertTrue(locations.isGlobalTitleDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify All restaurant", "Verify All restaurant","All restaurant is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
