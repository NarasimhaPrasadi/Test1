...
public class GpsDisableVerifyNoNetworkPopUpOnHeartTapOrderScreen 
{
	private ExtLogger log = new ExtLogger(GpsDisableVerifyNoNetworkPopUpOnHeartTapOrderScreen.class.toString());
	...
	@Test
	public void VerifyNoNetworkPopUp_OnHeartTapOrderScreenScript()
	{
		...
		try {
            ...
            home.WifiDisable();
            locations.isMobileOrderDisplayed();
            locations.FavouriteHeartPickUpScreen();      
            
            try {
        	    Assert.assertTrue(locations.isGlobalTitleDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify No Network popup", "Verify No Network popup", "No Network popup is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify No Network popup", "Verify No Network popup", "No Network popup is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
