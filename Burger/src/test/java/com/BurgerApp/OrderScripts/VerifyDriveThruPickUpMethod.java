...
public class VerifyDriveThruPickUpMethod 
{
	private ExtLogger log = new ExtLogger(VerifyDriveThruPickUpMethod.class.toString());
		...
		try {
           ...
           try {
               Assert.assertTrue(locations.isDriveThruDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify DriveThru", "Verify DriveThru","DriveThru Screen Verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify DriveThru", "Verify DriveThru", "DriveThru Screen Verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
