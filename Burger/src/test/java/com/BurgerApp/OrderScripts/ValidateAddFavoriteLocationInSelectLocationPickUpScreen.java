...
public class ValidateAddFavoriteLocationInSelectLocationPickUpScreen 
{
	private ExtLogger log = new ExtLogger(ValidateAddFavoriteLocationInSelectLocationPickUpScreen.class.toString());
	...
	@Test
	public void ValidateAddFavoriteLocationInSelectLocationPickUpScreenScript()
	{
		...
		try {
            ...
            try {
            	Assert.assertTrue(locations.isAddressLayoutDisplayed());
				reports.writeIntoFile(driver, testCaseName,  "Verify Location Search", "Verify Location Search", "Location Search is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify Location Search", "Verify Location Search", "Location Search is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
