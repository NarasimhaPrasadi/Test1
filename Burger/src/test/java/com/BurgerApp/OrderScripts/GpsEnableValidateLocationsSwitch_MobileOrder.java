...
public class GpsEnableValidateLocationsSwitch_MobileOrder 
{
	private ExtLogger log = new ExtLogger(GpsEnableValidateLocationsSwitch_MobileOrder.class.toString());
	...
	public void VerifyAssertListByOrderScreen()
	{
		...
		try {
            ...
            try {
				...
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileOrder button", "MobileOrder button", "MobileOrder is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify MobileOrder button", "MobileOrder button", "MobileOrder is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in Verified" +e.getMessage());
				Assert.assertTrue(false);
			}
