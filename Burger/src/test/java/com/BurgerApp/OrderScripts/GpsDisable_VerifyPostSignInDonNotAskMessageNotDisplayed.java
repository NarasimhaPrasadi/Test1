...
public class GpsDisable_VerifyPostSignInDonNotAskMessageNotDisplayed 
{
	private ExtLogger log = new ExtLogger(GpsDisable_VerifyPostSignInDonNotAskMessageNotDisplayed.class.toString());
	...
	@Test
	public void GpsDisable_VerifyPostSignInDonNotAskMessageNotDisplayedScript()
	{
		...
		try {
            ...
            home.Orders();
            home.startNewOrder();
            home.DoNotAskMessagePopUpGpsDisabled();
            locations.OrderScreenBack();
            home.Signout();
	        ...
	        try {
				...
				reports.writeIntoFile(driver, testCaseName,  "Verify SelectLocationPickupTitle", "Verify SelectLocationPickupTitle", "SelectLocationPickupTitle is verified", reports.pass, "", comlib.getCurrentTime());
			}
			catch(Exception e)
			{
				reports.writeIntoFile(driver, testCaseName,  "Verify SelectLocationPickupTitle", "Verify SelectLocationPickupTitle", "SelectLocationPickupTitle is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
				log.stepLog("Exception in SelectLocationPickupTitle" +e.getMessage());
				Assert.assertTrue(false);
			}
