...
public class ValidateBackButton_WithStartNewOrder 
{
	private ExtLogger log = new ExtLogger(ValidateBackButton_WithStartNewOrder.class.toString());
	...
	String moduleName = "Verify Location by back buuton";
	...
	@Test
	public void ValidateBackButton_WithStartNewOrder_Script()
	{
		try {
			...
			catch(Exception e)
			{
               ...
               try {
   				Assert.assertTrue(locations.isStartNewOrderButtonDisplayed());
   				reports.writeIntoFile(driver, testCaseName,  "Verify StartNewOrderButton", "Verify StartNewOrderButton", "StartNewOrderButton is verified", reports.pass, "", comlib.getCurrentTime());
   			}
   			catch(Exception e)
   			{
   				reports.writeIntoFile(driver, testCaseName,  "Verify StartNewOrderButton", "Verify StartNewOrderButton", "StartNewOrderButton is not verified", reports.fail, e.getMessage(), comlib.getCurrentTime());
   				log.stepLog("Exception in Verified" +e.getMessage());
   				Assert.assertTrue(false);
   			}
